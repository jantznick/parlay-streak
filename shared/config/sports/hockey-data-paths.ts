...
export function getPeriodNumbers(timePeriod: TimePeriod, gameData?: any): number[] {
  switch (timePeriod) {
    ...
    case 'P3':
      return [3];
    case 'OT':
      // Dynamically detect all OT periods (periods > 3)
      if (gameData) {
...
/**
 * Helper: Get all OT periods (periods > 3) from the game
 */
function getAllOTPeriods(gameData: any): number[] {
  const allPeriods = getAllPeriods(gameData);
  return allPeriods.filter(p => p > 3); // OT periods are 4+ for hockey
}
