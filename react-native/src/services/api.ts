...
class ApiService {
  ...
  // Dev-only admin login helper
  async devLoginAdmin(): Promise<ApiResponse<{ user: any }>> {
    return this.request<{ user: any }>('/api/auth/dev-login-admin', {
      method: 'POST',
      body: JSON.stringify({}),
    });
  }
  ...
  async updateBet(
    betId: string,
    updates: { bet_type?: string; config?: any; display_text_override?: string; priority?: number }
  ) {
